...
define('AMPFORWP_COMMENTS_PER_PAGE',  ampforwp_define_comments_number() );
	...
	function ampforwp_home_archive_rel_canonical() {
		...
		if ( $amp_url ) {
			...
			printf('<meta name="generator" content="AMP for WP <?php echo esc_attr(AMPFORWP_VERSION)?>" />', esc_url($amp_url));
		}
...
/*	include_once( ABSPATH . 'wp-admin/includes/plugin.php' );
	if( !is_plugin_active( 'amp-cta/amp-cta.php' ) )  {*/
		if( ($redux_builder_amp['enable-single-social-icons'] == true && is_single()) || (is_page() && true == $redux_builder_amp['ampforwp-page-sticky-social']))  { 
			...
			<?php }
	          ...
	          <amp-social-share type="twitter"
	                            ...
	                            data-param-url=""
                        		data-param-text="TITLE <?php echo esc_url($permalink).' '.ampforwp_translation( $redux_builder_amp['amp-translator-via-text'], 'via' ).' '.esc_html($data_param_data) ?>"
	          ></amp-social-share>
			  	<?php } ?>
		...
		<?php if($redux_builder_amp['enable-single-vk-share'] == true)  { ?>
			<a title="vkontakte share" <?php ampforwp_nofollow_social_links(); ?> href="http://vk.com/share.php?url=<?php echo $amp_permalink; ?>">
...
function ampforwp_view_nonamp(){
	...
	if ( $non_amp_url ) { ?><a class="view-non-amp" href="<?php echo esc_url(apply_filters('ampforwp_view_nonamp', $non_amp_url) ) ?>" <?php echo esc_attr($nofollow); ?>><?php echo esc_html__( $redux_builder_amp['amp-translator-non-amp-page-text'], 'accelerated-mobile-pages') ;?></a> <?php
	}
}
