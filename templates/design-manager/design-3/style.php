...
@font-face {
    ...
    font-style: normal;
    font-weight: 400;
    src:  local('Roboto Slab Regular'), local('RobotoSlab-Regular'), url('<?php echo esc_url($icon_url.'fonts/robotoslab/RobotoSlab-Regular.ttf'); ?>');
}
@font-face {
    font-family: 'Roboto Slab';
    font-display: swap;
    font-style: normal;
    font-weight: 700;
    src:  local('Roboto Slab Bold'), local('RobotoSlab-Bold'), url('<?php echo esc_url($icon_url.'fonts/robotoslab/RobotoSlab-Bold.ttf'); ?>');
}
...
@font-face {
    ...
    font-style: normal;
    font-weight: 400;
    src:  local('PT Serif'), local('PTSerif-Regular'), url('<?php echo esc_url($icon_url.'fonts/ptserif/PT_Serif-Web-Regular.ttf'); ?>');
}
@font-face {
    font-family: 'PT Serif';
    font-display: swap;
    font-style: normal;
    font-weight: 700;
    src:  local('PT Serif Bold'), local('PTSerif-Bold'), url('<?php echo esc_url($icon_url.'fonts/ptserif/PT_Serif-Web-Bold.ttf'); ?>');
}
#amp-comment-body p{
  margin:0px;
}
...
.amp-wp-content.the_content a{ 
  color:<?php echo ampforwp_sanitize_color( $link_color ); ?>; 
}
...
.design_3_wrapper .form-submit #submit{
  background:<?php echo ampforwp_sanitize_color( $colorscheme ); ?>; 
  color:<?php echo ampforwp_sanitize_color( $font_color ); ?>; 
}
...
<?php 
if( true == ampforwp_get_setting('amp-design-3-search-feature') ) { ?>
...
<?php }
if ( 1 == $redux_builder_amp['amp-enable-notifications'] || isset($redux_builder_amp['ampforwp-cta-subsection-notification-sticky']) && 1 == $redux_builder_amp['ampforwp-cta-subsection-notification-sticky'] ) {?>
/* Notifications */
...
amp-user-notification button{ padding: 8px 10px;color: <?php echo ampforwp_sanitize_color($font_color);?>; margin-left: 5px; border: 0; }
...
body a {  
  color:<?php echo ampforwp_sanitize_color( $colorscheme ); ?>; 
}

.amp-wp-content blockquote { border-color:  <?php echo ampforwp_sanitize_color( $colorscheme ); ?>; }
...
.comment-button-wrapper a, #pagination .next a, #pagination .prev a{ background: <?php echo ampforwp_sanitize_color($colorscheme);?>; }
.toast:after, .toast:before, .toast span{ background: <?php echo ampforwp_sanitize_color($colorscheme);?>; }
[class*=icono-], .headerlogo a{ color: <?php echo ampforwp_sanitize_color($colorscheme);?>; }
...
.callnow a:before { content: ""; position: absolute;top:-8px; width: 4px; height: 8px; border-width: 6px 0 6px 3px; border-style: solid; border-color:<?php echo ampforwp_get_setting('amp-opt-color-rgba-colorscheme-call','color','ampforwp_sanitize_color'); ?>; background: transparent; transform: rotate(-30deg); box-sizing: initial; border-top-left-radius: 3px 5px; border-bottom-left-radius: 3px 5px;
<?php if( true == ampforwp_get_setting('amp-rtl-select-option') ) { ?>
  left:0px;
<?php } else {?>
 right: 0px; 
