...
function amp_icon_css(){
	...
	if ( 1 == $redux_builder_amp['amp-design-selector'] || 2 == $redux_builder_amp['amp-design-selector'] || 3 == $redux_builder_amp['amp-design-selector'] ) { ?>
		@font-face {font-family: 'icomoon';font-style: normal;font-weight: normal;font-display: swap;src:  local('icomoon'), local('icomoon'), url('<?php echo plugin_dir_url(__FILE__) ?>icomoon/icomoon.ttf');}
...
function amp_featured_video($design_type='',$amp_thumnail = ''){
	...
	if(ampforwp_get_setting('ampforwp-featured-video')==true && !empty(ampforwp_get_setting('ampforwp-featured-video-metakey')) && !empty($youtubelink)){
		...
		if(!empty($videoID)){
			$featured_video = '<amp-youtube width="1000" height="563" layout="responsive" data-videoid="'.esc_attr($videoID).'"></amp-youtube>';
		}
	}else{
		if($design_type == 4 && !empty($amp_thumnail)){
			if(ampforwp_get_setting('single-design-type') == 1 || ampforwp_get_setting('single-design-type') == 4 || ampforwp_get_setting('single-design-type') == 7 ){
				if(is_array($amp_thumnail)){
					if( !empty($amp_thumnail['thumb_url']) && $amp_thumnail['show_image']){
						$featured_video = '<amp-img src="'.esc_url( $amp_thumnail['thumb_url'] ).'" width="'.esc_attr($amp_thumnail['thumb_width']).'" height="'.esc_attr($amp_thumnail['thumb_height']).'" layout="responsive" alt="'. esc_html(get_the_title()) .'" ></amp-img>';
					}
				}else{
			...
			if(is_array($amp_thumnail) ){
				if(!empty($amp_thumnail['thumb_url']) && $amp_thumnail['show_image']){
					$featured_video = '<amp-img src="'.esc_url( $amp_thumnail['thumb_url'] ).'" width="'.$amp_thumnail['thumb_width'].'" height="'.$amp_thumnail['thumb_height'].'" layout="responsive" alt="'. esc_html(get_the_title()) .'" ></amp-img>';
				}
			}else{
				$featured_video = $amp_thumnail;
			}
			...
			if(is_array($amp_thumnail) ){	
				if(!empty($amp_thumnail['thumb_url']) && $amp_thumnail['show_image']){
					$featured_video = '<amp-img src="'.esc_url( $amp_thumnail['thumb_url'] ).'" width="'.$amp_thumnail['thumb_width'].'" height="'.$amp_thumnail['thumb_height'].'" '.esc_attr($responsiveLayout).' alt="'. esc_html(get_the_title()) .'" ></amp-img>';
				}
			}else{
				$featured_video = $amp_thumnail;
			}
...
function amp_header_core(){
	...
	global $redux_builder_amp;
	$html_tag_attributes = AMP_HTML_Utils::build_attributes_string( $ampforwpTemplate->get( 'html_tag_attributes' ) );
...
function ampforwp_sanitize_archive_desc(){
	...
	if ( is_home() ) {
    	...
    	$bodyClass = 'amp-index amp-home'.esc_attr( $ampforwpTemplate->get( 'body_class' ) ); 
        ...
        if ( $ampforwp_custom_post_page == "page" && ampforwp_name_blog_page() ) {
			...
			if( in_array( ampforwp_name_blog_page() , $current_url_in_pieces )  ) {
				 $bodyClass = 'amp-index' .esc_attr( $ampforwpTemplate->get( 'body_class' ) ); 
			}  
		}
	...
	?><!doctype html>
	<html <?php echo ampforwp_amp_nonamp_convert('amp '); ?><?php echo AMP_HTML_Utils::build_attributes_string( $ampforwpTemplate->get( 'html_tag_attributes' ) ); ?>>
		<head>
		<meta charset="utf-8"> 

			<?php do_action('amp_experiment_meta', $ampforwpTemplate); ?>
		    <link rel="dns-prefetch" href="https://cdn.ampproject.org">
		    <?php do_action( 'amp_meta', $ampforwpTemplate ); ?>
		    <?php 
		...
		<?php do_action('amp_start', $ampforwpTemplate); ?>
		<?php do_action('ampforwp_body_beginning', $ampforwpTemplate);  
}
...
function amp_header(){
	...
	$ampforwpTemplate->load_parts( array( 'header' ) ); 
	do_action( 'amp_after_header', $ampforwpTemplate );
	do_action( 'ampforwp_after_header', $ampforwpTemplate );
...
function amp_footer(){
	...
	do_action( 'amp_before_footer', $ampforwpTemplate );
	do_action( 'amp_post_template_above_footer', $ampforwpTemplate );

	$ampforwpTemplate->load_parts( array( 'footer' ) );
...
function amp_loop_template(){
	global $ampforwpTemplate;
	$post_id = get_queried_object_id();
	do_action('amp_before_loop',$ampforwpTemplate);
	$ampforwpTemplate->load_parts( array( 'loop' ) );
	do_action('amp_after_loop',$ampforwpTemplate);
}
...
if ( empty( $post_id )) {
    ...
    if ( 1 == ampforwp_get_setting('amp-design-selector')  ) {
		do_action('ampforwp_inside_post_content_before',$ampforwpTemplate); 
	}
	else{
	...
	if ( ! $amp_custom_content_enable ) {
			...
			$ampforwp_the_content = $ampforwpTemplate->get( 'ampforwp_amp_content' );
		} 
	// Muffin Builder Compatibility #1455 #1893
	if ( function_exists('mfn_builder_print') && ! $amp_custom_content_enable ) {
		ob_start();
	  	mfn_builder_print( $ampforwpTemplate->get( 'post_id' ) );
	...
	do_action('ampforwp_after_post_content',$ampforwpTemplate); ?>
<?php }
