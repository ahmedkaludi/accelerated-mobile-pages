...
if ( is_admin() ) {
	...
	function ampforwp_plugin_notice() {
		...
		if ( ! defined( 'AMP__FILE__' ) ) {
            ...
            if ( "plugin-install" === $current_screen ->id || "dashboard_page_ampforwp-welcome-page" === $current_screen ->id || "ampforwp-welcome-page" === $current_screen ->id ) {
                return;
            }
            ...
            ?>
				...
				<div class="notice notice-warning is-dismissible ampinstallation">
						...
						<?php add_thickbox(); ?>
				        <p>
                        <strong><?php esc_html_e( 'AMP Installation requires one last step:','accelerated-mobile-pages' ); ?></strong> <?php esc_html_e( 'AMP by Automattic plugin is not active', 'accelerated-mobile-pages' ); ?>
				         <strong>	<span style="display: block; margin: 0.5em 0.5em 0 0; clear: both;"><a href="index.php?page=ampforwp-welcome-page"><?php esc_html_e( 'Continue Installation', 'accelerated-mobile-pages' ); ?></a>
                             </span> </strong>
				        </p>
				</div> <?php
			}
...
" id="gf_dashboard_message">
    ...
    font-weight: 300;
    top: -14px;
    font-size: 20px;"> <?php esc_html_e('AMP has been updated to '.$ampforwp_current_version, 'accelerated-mobile-pages' ); ?></div>
...
<div style="display:inline-block;float:right;height: 70px;background: #333;width: 50px;text-align: center;">
    ...
    font-weight: 300;
    background: #333;
    z-index: 100;" href="<?php echo esc_url(add_query_arg( 'ampforwp-dismiss', 'ampforwp_dismiss_admin_notices' )) ?>">X</a>
	    </div> 
