			...
			if ( ! defined( 'AMP_COMMENTS_VERSION' ) ) { ?>
				<div class="cmt-button-wrapper">
					<?php if ( comments_open() ) { 
						...
						?>
				    	<a href="<?php echo esc_url(ampforwp_comment_button_url()); ?>" <?php esc_html($nofollow) ?> title="<?php echo ampforwp_get_setting('amp-translator-leave-a-comment-text') ?>" ><?php echo  ampforwp_translation( $redux_builder_amp['amp-translator-leave-a-comment-text'], 'Leave a Comment'  ); ?></a> <?php
					}?>
				</div> <?php 
			} ?>
		</div><?php 
