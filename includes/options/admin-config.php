    ...
    if($extension['plugin_active_path'] != "" && is_plugin_active($extension['plugin_active_path']) ){
        ...
        if(isset($selectedOption['amp-license'][$pathExploded]['status']) && $selectedOption['amp-license'][$pathExploded]['status']==='valid'){
            $license_status = $selectedOption['amp-license'][$pathExploded]['status'];
             ...
             $verify = '<button type="button" id="'.$pathExploded.'" class="redux-ampforwp-ext-deactivate">Deactivate</button> <span class="ampforwp-ext-refresh" style="cursor:pointer" id="'.$pathExploded.'"><i class="dashicons-before dashicons-update"></i>Refresh</span>';
            ...
            if(isset($selectedOption['amp-license'][$pathExploded]['all_data']) && $selectedOption['amp-license'][$pathExploded]['all_data']!=""){
                ...
                if($remainingExpiresDays>0){
                    $amp_license_response = "<span class='license-tenure'>".$remainingExpiresDays." Days Remaining</span>. <a href='https://accounts.ampforwp.com/order/?edd_license_key=".$amplicense."&download_id=".$allResponseData['item_name']."'>Renew License</a>";
                }else{ $amp_license_response = "<span class='license-tenure'>Expired!</span> <a href='https://accounts.ampforwp.com/order/?edd_license_key=".$amplicense."&download_id=".$allResponseData['item_name']."'>Renew your license</a>"; }
            }
        }
