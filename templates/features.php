...
define('AMPFORWP_COMMENTS_PER_PAGE',  ampforwp_define_comments_number() );
	...
	function ampforwp_static_blog(){
		global $page;
		$modify_canonical = false;
		$get_front_page_reading_settings  = get_option('page_on_front');
		// Homepage support on   
    	$get_amo_homepage_support        =  ampforwp_get_setting('ampforwp-homepage-on-off-support');

		if ( 'page' == get_option( 'show_on_front') && is_front_page() && $get_front_page_reading_settings && $get_amo_homepage_support ){
			...
			<amp-facebook-like <?php echo "width=". $fblikewidth ."" ?> height=18 style="margin-bottom:-18px;"
	          ...
	          ></amp-social-share>
			  	...
			  	<?php if($redux_builder_amp['enable-single-whatsapp-share'] == true)  { ?>
							<a title="whatsapp share" <?php ampforwp_nofollow_social_links(); ?> href="whatsapp://send?text=<?php echo $amp_permalink; ?>">
...
function ampforwp_view_nonamp(){
	...
	if ( $non_amp_url ) { ?><a class="view-non-amp" href="<?php echo esc_url(apply_filters('ampforwp_view_nonamp_url', $non_amp_url) ) ?>" <?php echo esc_attr($nofollow); ?>><?php echo esc_html__( $redux_builder_amp['amp-translator-non-amp-page-text'], 'accelerated-mobile-pages') ;?></a> <?php
	}
}
