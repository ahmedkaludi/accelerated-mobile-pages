	...
	function ampforwp_amphtml_generator(){
      ...
      if ( is_archive() && !function_exists('amp_woocommerce_pro_add_woocommerce_support') ) {
		...
		if (!ampforwp_is_home() && !ampforwp_is_front_page() && !is_category() && !is_tag() && !is_singular( array('page', 'attachment', 'post'))){
			global $post_type;
			if (empty(ampforwp_get_setting('ampforwp-custom-type')) && !function_exists('amp_woocommerce_pro_add_woocommerce_support')) {
				return;
			}
			if(is_array(ampforwp_get_setting('ampforwp-custom-type')) && $post_type != ampforwp_get_setting('ampforwp-custom-type')[0] && !function_exists('amp_woocommerce_pro_add_woocommerce_support')){
				return;
			}
		}
...
function ampforwp_auto_flush_on_save($redux_builder_amp) {
	if ( ampforwp_get_setting('amp-on-off-for-all-pages') == 1 || ampforwp_get_setting('ampforwp-archive-support') == 1 || ampforwp_get_setting('fb-instant-article-switch') == 1 ) {
		global $wp_rewrite;
...
function ampforwp_is_non_amp( $type="" ) {
	...
	$mob_pres_link = ampforwp_mobile_redirect_preseve_link();
	if (""===$type  && (isset( $redux_builder_amp['ampforwp-amp-takeover']) && true == $redux_builder_amp['ampforwp-amp-takeover'] || $mob_pres_link==true) ) {
		$non_amp = true;
		// Check for Posts
...
if(ampforwp_is_gutenberg_active()){
...
if ( ! function_exists('ampforwp_gutenberg_css') ) {
	function ampforwp_gutenberg_css(){
		...
		.wp-block-button a {background-color: <?php echo $background;?>;border-radius: 28px;color: inherit;display: inline-block;padding: 12px 24px;}
...
function ampforwp_bulktool_takeover($data){
  ...
  if(function_exists('ampforwp_mobile_redirect_preseve_link')){
	...
	if(is_page() && $bulk_option == "hide" && !isset($ampforwp_stored_meta) && ( true == ampforwp_get_setting('ampforwp-amp-takeover') || true == ampforwp_get_setting('ampforwp-amp-convert-to-wp') || $mob_pres_link==true)){
		remove_action( 'wp_head', 'ampforwp_home_archive_rel_canonical', 1 );
		return false; 
	}
	return $data;
...
if(!function_exists('ampforwp_default_fallback_img_src_url')){
