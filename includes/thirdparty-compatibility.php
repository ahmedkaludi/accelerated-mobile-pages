...
function ampforwp_thirdparty_compatibility(){
	...
	if(function_exists('hotwp_get_option') && is_category()){
		add_filter('single_cat_title' , true );
	...
	**/
	...
	//Sahifa theme video compatibility in amp #3138
	add_filter('the_content', 'ampforwp_sahifa_theme');
...
function ampforwp_non_amp_gallery_testimonial($matches){
	$images =  $matches[3];
	$images = preg_replace_callback("/<img(.*?)>/", function($m){
		return '<li class="mySlides-tsml">'.$m[0]. /* $m[0] is already sanitized, XSS OK */'</li>';
	}, $images);
	$imagesHTML = '<ul class="slideshow-container-tsml">'.$images. /* $images is already sanitized, XSS OK */'<a class="nonamp-prev" onclick="plusSlides(-1)">&#10094;</a>
<a class="nonamp-next" onclick="plusSlides(1)">&#10095;</a></ul>';
	return $imagesHTML;
}
...
function ampforwp_sahifa_theme($content){
	global $post;
...
function ampforwp_hotwp_add_cat_title(){
	$category = get_the_category();
	echo '<div class="cntr amp-wp-content amp-wp-article"><h2 class="amp-archive-title"><br><br>' . $category[0]->cat_name . '</h2></div>';
}