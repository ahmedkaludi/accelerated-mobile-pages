...
function ampforwp_title_callback( $post ) {
	...
	if ( get_option('page_on_front') == $post->ID && false == $redux_builder_amp['amp-frontpage-select-option'] ) {
		echo sprintf(('<p class="afp"><b>%s: %s </b> <a class="" href="%s">%s</a></p>'), esc_html__('Note', 'accelerated-mobile-pages'),esc_html__('This is not your AMP FrontPage, you can','accelerated-mobile-pages'),admin_url("admin.php?page=amp_options&tabid=opt-text-subsection#redux_builder_amp-amp-frontpage-select-option"),esc_html__('set one from here','accelerated-mobile-pages'));
	}
...
function ampforwp_meta_redirection_status(){
	...
	if ( (! is_404() && ampforwp_is_search_has_results())  && (is_singular() ||  ampforwp_is_front_page())) {
		...
		if(isset($ampforwp_post_metas['ampforwp-redirection-on-off'])){
...
/*	include_once( ABSPATH . 'wp-admin/includes/plugin.php' );
	if( !is_plugin_active( 'amp-cta/amp-cta.php' ) )  {*/
			 	...
			 	if( false == ampforwp_get_setting('ampforwp-facebook-like-data-action')) {
					...
					if ( $facebook_like_url ) { ?>
						<amp-facebook-like width=90 height=18 style="margin-bottom:-18px;"
						    ...
						    <?php ampforwp_nofollow_social_links(); ?> data-href="<?php echo esc_url($facebook_like_url); ?>">
						</amp-facebook-like>
					<?php }
				}else{
		...
		<?php } ?>
	          ...
	          ></amp-social-share>
			  	...
			  	<?php if($redux_builder_amp['enable-single-email-share'] == true)  { ?>
			    	...
			    	aria-label="email" <?php ampforwp_nofollow_social_links(); ?>></amp-social-share>
			  	<?php } ?>
			  	...
			  	<?php if($redux_builder_amp['enable-single-whatsapp-share'] == true)  { ?>
							<a title="whatsapp share" <?php ampforwp_nofollow_social_links(); ?> href="https://api.whatsapp.com/send?text=<?php echo $amp_permalink; ?>">
		...
		<?php if($redux_builder_amp['enable-single-vk-share'] == true)  { ?>
			<a title="vkontakte share" <?php ampforwp_nofollow_social_links(); ?> href="http://vk.com/share.php?url=<?php echo $amp_permalink; ?>" target="_blank">
		...
		<?php if($redux_builder_amp['enable-single-odnoklassniki-share'] == true)  { ?>
			<a title="odnoklassniki share" <?php ampforwp_nofollow_social_links(); ?> href="https://ok.ru/dk?st.cmd=addShare&st._surl=<?php echo $amp_permalink; ?>" target="_blank">
...
function ampforwp_global_head_scripts($data){
	...
	if( $redux_builder_amp['amp-header-text-area-for-html'] ) {
		$allscripts = $redux_builder_amp['amp-header-text-area-for-html'];
...
function ampforwp_header_html_output() {
  	global $redux_builder_amp;
  	if( $redux_builder_amp['amp-header-text-area-for-html'] ) {
  		$allhtml = $redux_builder_amp['amp-header-text-area-for-html'] ;
...
function ampforwp_meta_description() {
	global $redux_builder_amp;
	if ( ! $redux_builder_amp['ampforwp-seo-meta-description'] || 'rank_math' == ampforwp_get_setting('ampforwp-seo-selection') ) {
		return;
	}
	$desc = "";
...
function ampforwp_view_nonamp(){
	...
	if ( $non_amp_url ) { ?><a class="view-non-amp" href="<?php echo esc_url(apply_filters('ampforwp_view_nonamp_url', $non_amp_url) ) ?>" <?php echo esc_attr($nofollow); ?> title="<?php echo ampforwp_get_setting('amp-translator-non-amp-page-text') ?>"><?php echo esc_html__( $redux_builder_amp['amp-translator-non-amp-page-text'], 'accelerated-mobile-pages') ;?></a> <?php
	}
}
	...
	if(is_single()){
			...
			if($post_paginated_page){
				...
				if('' == $permalink_structure){
					...
					$new_url = trailingslashit(trailingslashit($url).$post_paginated_page);
				}
...
function ampforwp_add_vuukle_scripts( $data ) {
	...
	if ( isset($redux_builder_amp['ampforwp-vuukle-comments-support'])
		 ...
		 && $display_comments_on
			...
			if (ampforwp_get_setting('ampforwp-vuukle-Ads-before-comments')
