...
?>
amp-sidebar{ width: 280px;font-family: 'Roboto Slab', serif;background:<?php echo sanitize_hex_color($menubgcolor);?>; }
.amp-sidebar-image{ line-height: 100px; vertical-align:middle; }
.amp-close-image{ top: 15px; left: 225px; cursor: pointer; }
.navigation_heading{ padding: 20px 20px 15px 20px; color:<?php echo $menulblcolor; ?>; font-size: 10px; font-family: sans-serif; text-transform: uppercase; letter-spacing: 1px; border-bottom: 1px solid <?php echo sanitize_hex_color($menubdrcolor); ?>; display: inline-block; width: 100%}
...
color:<?php echo sanitize_hex_color($navmenucolor); ?>; width:100% }
.amp-menu li{position:relative}
.toggle-navigationv2 ul.amp-menu li a:hover{background:<?php echo sanitize_hex_color($submenuhovercolor); ?>;color:<?php echo sanitize_hex_color($navmenucolor); ?>;}
.amp-menu li.menu-item-has-children ul{display:none;margin:0;position:relative;
background:<?php echo sanitize_hex_color($submenucolor);?>;
}
.amp-menu li.menu-item-has-children .sub-menu li a span:before{
    ...
    left: -6px;
    font-size: 10px;
    color:<?php echo sanitize_hex_color($menulblcolor); ?>;
    top: -2px;
    z-index: 10000;
    line-height: 1;
}
...
.amp-menu .toggle:after{content:'\25be';position:absolute;padding: 12px 20px 10px 30px;right:0;font-size:13px;color:<?php echo sanitize_hex_color($navmenucolor); ?>;top:6px;z-index:10000;line-height:1}
.toggle-navigationv2 .social_icons{ margin-top: 25px; border-top: 1px solid <?php echo sanitize_hex_color($menubdrcolor); ?>; padding: 25px 0px; color: #fff; width: 100%; }
...
#header, .headerlogo a{ background:<?php echo sanitize_hex_color($headercolor); ?>  }
...
.toast:after, .toast:before, .toast span{ background: <?php echo sanitize_hex_color($headerelements);?>; }
[class*=icono-], .headerlogo a{ color: <?php echo sanitize_hex_color($headerelements);?>; }
