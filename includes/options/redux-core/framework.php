...
//                            }
            ...
            public function section_menu( $k, $section, $suffix = "", $sections = array() ) {
                ...
                if ( isset ( $section['type'] ) && $section['type'] == "divide" ) {
                    ...
                    $string .= '<li '.esc_html($opt_visible).' id="' . esc_attr( $k . $suffix ) . '_section_group_li" class="'.esc_attr($opt_visible_class).' redux-group-tab-link-li '.$addClass.'' . esc_attr( $hide_section ) . esc_attr( $section['class'] ) . esc_attr( $subsectionsClass ) . ' ' . strtolower( wp_kses_post( $section['id'] )) . '" '.ampforwp_css_sanitizer($style).'>';
